...
# Migrated to pure PyTorch - removed mmengine dependencies
import logging
...
def test_resnet():
    ...
    print("\n4. Testing ResNet18...")
    model_18 = ResNet(depth=18, num_stages=4, out_indices=(3,))
    with torch.no_grad():
        out_18 = model_18(x)
    print(f"   ResNet18 output shape: {out_18.shape}")

    print("\n5. Testing ResNet101...")
    model_101 = ResNet(depth=101, num_stages=4, out_indices=(2, 3))
    with torch.no_grad():
        outs_101 = model_101(x)
    print(f"   ResNet101 outputs: {len(outs_101)} tensors")

    print("\n✓ All tests passed!")


if __name__ == '__main__':
    # To run this test: python -m reimplementation.models.backbones.resnet
    test_resnet()