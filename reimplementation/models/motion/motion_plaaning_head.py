...
from ..utils.model_utils import bias_init_with_prob, linear_relu_ln, reduce_mean, force_fp32
from ..utils.builders import build_from_cfg, COMPONENT_REGISTRY
from ..common.box3d import *

from ..common.attention import gen_sineembed_for_position
from ..common.instance_bank import topk
...
class MotionPlanningHead(nn.Module):
    def __init__(
        ...
        # =========== build modules ===========
        self.instance_queue = build_from_cfg(instance_queue)
        self.motion_sampler = build_from_cfg(motion_sampler)
        self.planning_sampler = build_from_cfg(planning_sampler)
        self.motion_decoder = build_from_cfg(motion_decoder)
        self.planning_decoder = build_from_cfg(planning_decoder)
        ...
        self.layers = nn.ModuleList(
            [
                build_from_cfg(self.op_config_map.get(op, None))
        ...
        self.motion_loss_cls = build_from_cfg(motion_loss_cls)
        self.motion_loss_reg = build_from_cfg(motion_loss_reg)
        self.plan_loss_cls = build_from_cfg(plan_loss_cls)
        self.plan_loss_reg = build_from_cfg(plan_loss_reg)
        self.plan_loss_status = build_from_cfg(plan_loss_status)
